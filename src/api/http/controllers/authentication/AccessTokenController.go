...
func (controller *AccessTokenController) RefreshAccessToken(c *gin.Context) {
	...
	var req authentication2.RefreshAccessTokenRequest
	...
	ctx := context.WithValue(c.Request.Context(), consts.OwnerType, models.UserRole)
	ctx = context.WithValue(ctx, consts.RequestUuid, c.GetString("request-uuid"))

	jwt, err := controller.AccessTokenService.RefreshAccessTokens(ctx, req.RefreshToken, models.UserRole)
	if err != nil {
