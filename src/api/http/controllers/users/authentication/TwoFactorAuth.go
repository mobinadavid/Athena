...
func twoFaContext(c *gin.Context, ownerType string) (context.Context, *utils.Auth) {
	auth := utils.GetAuthData(c)
	ctx := context.WithValue(c.Request.Context(), consts.OwnerType, ownerType)
...
func (controller *TwoFAController) Status(c *gin.Context) {
	ctx, auth := twoFaContext(c, models.UserRole)
